...
Massive and Twelve Data both expose technical indicator endpoints. STORY-007 decided
that deterministic NookScout calculations are the MVP source of truth for moving
averages, RSI, MACD, ATR, relative volume, support/resistance, relative strength, setup
levels, risk/reward, setup classification, and failure conditions.

Provider-supplied indicators may be used only as provider context or manual comparison
data during MVP development. They must not drive setup scoring or replace internal
calculations unless a future PRD explicitly approves fallback behavior. Formula
ownership, fixture expectations, numerical tolerance guidance, and versioning rules are
documented in [scoring-methodology.md](scoring-methodology.md).
